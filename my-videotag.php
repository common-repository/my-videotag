...
function activate_mvt() {
	...
	$mvt_opts2 =array('width' => '640',
	                   'height' => '390',
					   'style' => 'border:1px solid #CCCCCC;margin: 0 auto 10px;padding: 5px;height: 390px;',
					   'comments' => '1',
					   'authormoney' => '1');
	if ($mvt_opts1) {
...
if (isset($mvt['authormoney'])) {
function mvt_dominio_url($uri){
	preg_match("/^(http:\/\/)?([^\/]+)/i", $uri, $matches);
	$host = $matches[2];
	preg_match("/[^\.\/]+\.[^\.\/]+$/", $host, $matches);
	return $matches[0];	   
}
function mvt_parse_link_money($matches){
	if (( mvt_dominio_url($matches[3]) != mvt_dominio_url($_SERVER["HTTP_HOST"]) )) {
		return '<a href="http://2f5fca09.linkbucks.com/url/' . $matches[2] . '//' . $matches[3] . '"' . $matches[1] . $matches[4] . ' target="_blank">' . $matches[5] . '</a>';	 
	} else {
		return '<a href="' . $matches[2] . '//' . $matches[3] . '"' . $matches[1] . $matches[4] . '>' . $matches[5] . '</a>';
	}
}
function mvt_link_money($text) {	
	$pattern = '/<a (.*?)href="(.*?)\/\/(.*?)"(.*?)>(.*?)<\/a>/i';
	$text = preg_replace_callback($pattern,'mvt_parse_link_money',$text);
	return $text;
}
add_filter('the_content', 'mvt_link_money', 999);
add_filter('the_excerpt', 'mvt_link_money', 999);
add_filter('comment_text', 'mvt_link_money', 999);
}
